# Apple Studio Display (2026) and Apple Studio Display XDR (2026) Sensors Fix Windows Driver

## What is it?

This is an unofficial Windows driver that fixes the Apple Display HID Orientation Sensor and Apple Display HID Ambient Light Sensor on the Apple Studio Display (2026) (`VID_05AC&PID_1118`) and the Apple Studio Display XDR (2026) (`VID_05AC&PID_1116`) to work with the Apple Boot Camp Control Panel and third-party apps such as [StudioBrightness++](https://github.com/LitteRabbit-37/Studio-Brightness-PlusPlus).
...
E.g., when an Apple Studio Display XDR (2026) (`VID_05AC&PID_1116`) is connected to a Windows PC using a Thunderbolt/USB-C to Thunderbolt/USB-C cable or a DisplayPort + USB to USB-C cable, the display works via DisplayPort Alt Mode, the built-in USB hub, camera, speakers, and microphone are detected as standard USB devices and work out of the box.
...
For Apple Studio Display (2026) (`VID_05AC&PID_1118`):

1. Download and extract the latest successful build artifact from the [Actions](https://github.com/FluorescentHallucinogen/apple-studio-display-xdr-sensors-fix-windows-driver/actions) tab.
2. Run as administrator the `InstallAppleStudioDisplaySensorsDrivers.bat`.

For Apple Studio Display XDR (2026) (`VID_05AC&PID_1116`):
...
3. Run as administrator the `InstallAppleStudioDisplayXDRSensorsDrivers.bat`.
4. Reboot the system.

## How does it work?

The driver for Apple Studio Display (2026) (`VID_05AC&PID_1118`) (`AppleStudioDisplayAmbientLightOrientationHIDSensorsNull`) is a null driver.

It matches the HID Ambient Light Sensor (`HID\VID_05AC&PID_1118&MI_08`) and HID Orientation Sensor (`HID\VID_05AC&PID_1118&MI_09`) interfaces, assigns them friendly names, and installs no service, allowing apps to read raw HID data.

The driver for Apple Studio Display XDR (2026) (`VID_05AC&PID_1116`) consists of two parts.
